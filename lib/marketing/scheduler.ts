...
async function loadSubscriber(supabase: DB, id: string): Promise<SubscriberLite | null> {
  ...
  // 20260702120000 on the marketing branch; the live DB already has it) — a
  // subscriber without a campaign can't ride the campaign scheduler, so treat
  // it like a missing subscriber rather than faking an id into analytics.
  if (!data || !data.campaign_id) return null;
  return { id: data.id, email: data.email, status: data.status as SubscriberStatus, campaign_id: data.campaign_id };
}
