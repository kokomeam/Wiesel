/**
 * Structured (renderer-owned) layouts — the SCHEMA registry: each layout's
 * typed Zod CONTENT SCHEMA (the definition of the fillable slots) joined with
 * the zod-free metadata/seed registry in ./structuredLayoutsCore.ts (which
 * this module re-exports, so every existing import keeps working unchanged).
 * The render path (SlideView / StructuredSlide / layout components) binds to
 * the CORE module only — viewing a slide never loads zod (PERF-1 D1).
 *
...
import {
  LIMITS,
  STRUCTURED_LAYOUT_META,
  type StructuredLayoutMeta,
} from "./structuredLayoutsCore";

/* Everything zod-free re-exports from the core registry — existing consumers
   of this module stay byte-identical. */
export {
  DEFAULT_LAYOUT_POINT_CAPACITY,
  ITEM_BOUNDS,
  LIMITS,
  PLANNABLE_LAYOUT_IDS,
  STRUCTURED_LAYOUT_IDS,
  STRUCTURED_LAYOUT_META,
  findStructuredLayoutMeta,
  isStructuredLayoutId,
  layoutPointCapacity,
  structuredLayoutCatalog,
  type StructuredLayoutMeta,
} from "./structuredLayoutsCore";
...
export interface StructuredLayoutDef extends StructuredLayoutMeta {
  /** Strict content schema (the fillable-slot definition + length limits). */
  schema: z.ZodTypeAny;
}

/** Each layout's strict content schema, joined with the core metadata below. */
const LAYOUT_SCHEMAS: Record<StructuredLayoutId, z.ZodTypeAny> = {
  process_steps: ProcessContentSchema,
  key_concept: KeyConceptContentSchema,
  metrics_overview: MetricsContentSchema,
  code_walkthrough_steps: CodeWalkthroughContentSchema,
  section_break: SectionBreakContentSchema,
  concept_example: ConceptExampleContentSchema,
  outline_list: OutlineListContentSchema,
  prose: ProseContentSchema,
  comparison_columns: ComparisonColumnsContentSchema,
  comparison_matrix: ComparisonMatrixContentSchema,
  diagram: DiagramContentInputSchema,
  illustration: IllustrationContentSchema,
  image_reference: ImageReferenceContentSchema,
  image_supporting: ImageSupportingContentSchema,
};

export const STRUCTURED_LAYOUTS: StructuredLayoutDef[] = STRUCTURED_LAYOUT_META.map((meta) => ({
  ...meta,
  schema: LAYOUT_SCHEMAS[meta.id],
}));
...
export function validateStructuredContent(layoutId: string, content: unknown): string[] {
  ...
  if (!layout) {
    return [`Unknown structured layout "${layoutId}". Valid: ${STRUCTURED_LAYOUTS.map((l) => l.id).join(", ")}.`];
  }
