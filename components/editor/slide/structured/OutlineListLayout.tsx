...
export function OutlineListLayout({ content, ctx }: { content: OutlineListContent; ctx: StructuredCtx }) {
  ...
  // Scale type to the item count so 5 items × 2 sub-points stays readable.
  ...
  return (
    <div className="absolute inset-0">
      ...
      {/* Items — a flowing column: each item grows to fit its text + sub-points,
          evenly distributed in the region; long items wrap (never clip). */}
      <div
        className="absolute flex flex-col justify-center"
        style={{ left: 80, right: 80, top: REGION_TOP, bottom: REGION_BOTTOM, gap: 18 }}
      >
        {items.map((item, i) => {
