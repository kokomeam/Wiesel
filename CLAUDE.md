...
- **Planned backend (NOT built):** Supabase (Postgres + auth + storage),
  Stripe, LLM orchestration (the PRD names LangChain / GPT-4o / Claude).

Everything so far is **front-end skeleton with mock data** — no backend, no
auth, no real AI calls. All buttons like Publish/Generate/Export are
presentational.
...
`tailwind.config.*`) · `framer-motion` · `lucide-react`. npm. No git repo
initialized. Dev: `npm run dev` (localhost:3000) · `npm run build` ·
`npm run lint` (all currently green/clean).
...
3. Supabase wiring (the course doc model was designed to map: stable ids,
   integer `order`, ISO timestamps, blocks as jsonb); auth; then Stripe.
